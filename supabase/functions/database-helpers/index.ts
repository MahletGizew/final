  ...
  try {
    ...
    if (action === 'reset_profile') {
      ...
      if (profilesTableExists) {
        ...
        const { data: profileReset, error: profileError } = await supabase.from('user_profiles').delete().eq('id', userId);
        resetResults.push({
          table: 'user_profiles',
      ...
      if (activitiesTableExists) {
        ...
        const { data: activitiesReset, error: activitiesError } = await supabase.from('user_activities').delete().eq('user_id', userId);
        resetResults.push({
      ...
      if (examsTableExists) {
        ...
        const { data: examsReset, error: examsError } = await supabase.from('user_exams').delete().eq('user_id', userId);
        resetResults.push({
          table: 'user_exams',
          success: !examsError
        });
      ...
      if (sessionsTableExists) {
        ...
        const { data: sessionsReset, error: sessionsError } = await supabase.from('study_sessions').delete().eq('user_id', userId);
        resetResults.push({
      ...
      if (progressTableExists) {
        ...
        const { data: progressReset, error: progressError } = await supabase.from('user_progress').delete().eq('user_id', userId);
        resetResults.push({
          table: 'user_progress',
      ...
      const { data: newProfile, error: newProfileError } = await supabase.from('user_profiles').upsert({
        id: userId,
        display_name: '',
        total_study_time: 0
      }).select();
      resetResults.push({
        table: 'user_profiles_create',
